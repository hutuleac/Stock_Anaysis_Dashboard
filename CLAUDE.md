...
- **Card layout (read top-down, no cross-referencing):** status badge + `reasons[0]` verdict sentence once at the top → Timing | Quality side by side (`lg:grid-cols-2`, stacked on phones), Revenue (5y) under Quality. Each section is a total + band hint, then **one row per component**: label · fill bar · `score/max` · `+gap left`, with the reading behind it on an indented second line. Rows come from `timingRows` / `qualityRows` (longTermIndicators.js) sorted by gap, biggest first — the top row *is* what the entry is waiting on, so there's no separate "waiting on" line, no free-floating signals list and no colour legend. Null components sink to the bottom.
