...
- **Card layout (read top-down, no cross-referencing):** status badge + `reasons[0]` verdict sentence once at the top → Timing section → Quality section → Revenue (5y). Each section is a total + band hint, then **one row per component**: label · fill bar · `score/max` · `+gap left` · the reading behind it. Rows come from `timingRows` / `qualityRows` (longTermIndicators.js) sorted by gap, biggest first — the top row *is* what the entry is waiting on, so there's no separate "waiting on" line, no free-floating signals list and no colour legend. Null components sink to the bottom.
