...
src/lib/
  ...
  defaultLists.js     — HARDCODED_DEFAULTS (= repo-root watchlist.json) + HARDCODED_ETFS; plain module so scripts/snapshot.mjs can import it
  snapshot.js         — seedSnapshot(): copies published snapshot.json cache entries into localStorage (newer ts wins)
  ...
  api/
    ...
    twelvedata.svelte.js — TwelveData candles (optional, rate-limited)
...
- **Card layout (read top-down, no cross-referencing):** status badge + `reasons[0]` verdict sentence once at the top → Timing | Quality side by side (`lg:grid-cols-2`, stacked on phones) → "Why this score" + trade window | Revenue (5y) (same 2-col grid; full width when no revenue data). Each section is a total + band hint, then **one row per component**: label · fill bar · `score/max` · `+gap left`, with the reading behind it on an indented second line. Rows come from `timingRows` / `qualityRows` (longTermIndicators.js) sorted by gap, biggest first — the top row *is* what the entry is waiting on, so there's no separate "waiting on" line, no free-floating signals list and no colour legend. Null components sink to the bottom.
- **`computeTimingScore` returns `notes`** — the same `signals` / `warnings` strings keyed by component (`{ text, warn }`), so a warning like "Broad market trend is still negative" sits on the Market row. `signals` / `warnings` stay flat for export and the scan panel.
